    ...
    **/
    public function create_transaction() {
        ...
        $fields = [
            ...
            'trans_total'     => $this->order->get_total(),
            'files'           => '',
        ...
        if ( 'completed' == $order_status ) {
            ...
            $order_note_mesg = sprintf( __( 'ERP Accounting: Payment: %s created successfully', 'erp-pro' ), $fields['invoice_number'] );
            ...
            $fields['due']            = $this->order->get_total();
            ...
            if ( $already_completed == 'completed' ) {
                ...
                $message = __( 'status changed to ', 'erp-pro' );
            } else {
                $message = __( 'created successfully and status set as ', 'erp-pro' );
            }

            if ( 'processing' == $order_status ) {
                $fields['status'] = 'awaiting_payment';
                $order_note_mesg = sprintf( __( 'ERP Accounting: Invoice: %s %s awaiting payment', 'erp-pro' ), $fields['invoice_number'], $message );
            } else if ( 'cancelled' == $order_status ) {
                $fields['status'] = 'void';
                $order_note_mesg = sprintf( __( 'ERP Accounting: Invoice: %s %s void ', 'erp-pro' ), $fields['invoice_number'], $message );
            } else {
                $fields['status'] = 'awaiting_approval';
                $order_note_mesg = sprintf( __( 'ERP Accounting: Invoice: %s %s awaiting approval ', 'erp-pro' ), $fields['invoice_number'], $message );
            }
        }
        ...
        if ( 'completed' == $this->order->get_status() && $transaction_id ) {
            ...
            foreach ( $items as $key => $item ) {
                ...
                $this->line_items[] = [
                    ...
                    'tax_amount'  => isset( $item['line_tax'] ) ? $item['line_tax'] : 0,
                    'tax_journal' => 0
                ];
            }
